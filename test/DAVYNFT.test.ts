...
describe("DAVYNFT Tests", function () {
  ...
  it("Concatenate hash", async function () {
    ...
    for (let i = 0; i <= 100; i++) {
      ...
      const actualHash = await nftContract.connect(nftContractOwner).concatenateHash(i == 0 ? ethers.constants.HashZero : '0x' + runningHash.toString('hex'), '0x' + value.toString('hex'))
  ...
  it("Calculate hash", async function () {
    ...
    actualHash = await nftContract.connect(nftContractOwner).getNextTokenHash(value)
    ...
    for (let i = 1; i < 20; i++) {
      ...
      await nftContract.connect(nftContractOwner).setNextTokenHash(expectedHash)

      value = Buffer.from(`this is a test ${i}`)

      actualHash = await nftContract.connect(nftContractOwner).getNextTokenHash(value)
      expectedHash = keccak256(Buffer.concat([expectedHash, value]))
  ...
  it("Mint", async function () {
    ...
    const runningHash = keccak256(tokenHash)
    const buyerBalanceBefore = Number(await ethers.provider.getBalance(buyer.address))
    const ownerBalanceBefore = Number(await ethers.provider.getBalance(nftContractOwner.address))
    const mintTx = await nftContract.connect(buyer).mint(1, 984, '0x' + tokenHash.toString('hex'), '0x' + runningHash.toString('hex'), { value: PRICE });
    const mintTxReceipt = await mintTx.wait(1);
    const args = mintTxReceipt.events[2].args
    const seller = args.nftAddress;
    const buyerAddress = args.buyer
    const tokenId = Number(args.tokenId)
    const buyerBalanceAfter = Number(await ethers.provider.getBalance(buyer.address));
    const ownerBalanceAfter = Number(await ethers.provider.getBalance(nftContractOwner.address))

    console.log('buyer balance before:', buyerBalanceBefore, 'after:', buyerBalanceAfter, 'difference:', buyerBalanceAfter - buyerBalanceBefore)
    console.log('owner balance before:', ownerBalanceBefore, 'after:', ownerBalanceAfter, 'difference:', ownerBalanceAfter - ownerBalanceBefore)
